                       ...
                       repeated = FALSE) 
{
  ...
  if (repeated == FALSE) {
    ...
    id_drop <- future_apply(crossTab, 2, FUN = function(x) any(x))
...
compositionbar_plot <- function(df, 
  ...
  if(draw_connection_line){
    # data prepare
    y_start <- future_apply(my_df,2, cumsum)
    y_end <- y_start[,-1]
    ...
    if( polygon_index < 0 || polygon_index > length(col_names)){
      ...
      switch(polygon_type,
             "convex" = {
               ...
               # find the convex
               appliedData <- future_lapply(splitData, function(df){
...
tidy_proteingroups_3 <- function(proteinGroups = NULL, # data.frame
  ...
  if (filter_rows_with_all_missing_values){
    switch(keep_data_type,
           "intensity" = {index_all_zero <- future_apply(proteinGroups_filtered,1,function(x)all(x == 0))},
           "ratio" = {index_all_zero <- future_apply(proteinGroups_filtered,1,function(x)all(is.na(x)))}
    )
    #
...
add.alpha <- function(col, alpha=1){
  ...
  future_apply(sfuture_apply(col, col2rgb)/255, 2, 
        function(x) 
...
remove_allNA_rows <- function(df){
  ...
  if(any(future_apply(df,1,function(x)all(is.na(x))))){ # if there is any all NA rows
    df[-which(future_apply(df,1,function(x)all(is.na(x)))),]
  }else{
    df
  }
...
remove_allNA_columns <- function(df){
  if(any(future_apply(df,2,function(x)all(is.na(x))))){ # if there is any all NA columns
    df[,-which(future_apply(df,2,function(x)all(is.na(x))))]
  }else{
    df
  }
  # this function can deal with data.frame and data matrix
...
recode_for_sankey <- function(df){
  ...
  df_recode<- as.data.frame(future_apply(df, 2, function(x) {x <- car::recode(x,expression)}))
...
table2lPercents_by_col <- function (data_matrix, digits = 2){
  ...
  per <- t(future_apply(data_matrix, 1, function(x) x/sums))
...
ggsimpleline <- function(data_matrix, 
  ...
  if(spline_smooth){
    ...
    if(!is.null(x_cord)){
      ...
      df_smooth<- data.frame(future_apply(df, 2,function(x){spline(as.numeric(x_cord),x)$y}))
      ...
      df_smooth<- data.frame(future_apply(df, 2,function(x){spline(x)$y}))
      m_smooth<- reshape2::melt(as.matrix(df_smooth))
