    ...
    if(input$map_grouping_data && !is.null(input$select_group_mapping) ){
      ...
      output$ui_select_color <- renderUI({
        ...
        if(!is.null(input$color_theme)){
          ...
          colorinput_ui_color_list <- lapply(1:length(groups), function(i) {
            tags$div(
    ...
    if(input$button_plot > 0){
      ...
      isolate({
        ...
        if(input$map_grouping_data && !is.null(input$select_group_mapping)){
          ...
          mycolor_discret <- unlist(lapply(1:number_of_groups, function(i){
        ...
        if(!is.null(input$select_linear_mapping)){
          ...
          colors <-  unlist(lapply(1:length(colors),function(x) add.alpha(colors[x], color_tranparent_scale_factor[x])))
        }
...
#   
#   list_names <<- reactive({
#     unlist(lapply(textinput_ids, function(x){input[[x]]}))
#   }) 
#   
#   # for the inital two list: inputlist1, inputlist2
#   
#   list_textArea_input <<- reactive({
#     t <-lapply(textAreaInput_ids, function(x){unlist(strsplit(input[[x]], "\n"))})
...
#     !any(unlist(lapply(list_textArea_input(), function(x){length(x) == 0})))
#   })
#   
#   
#   #  __ load sample data
...
#       
#       list_textArea_input <<- reactive({
#         t <-lapply(textAreaInput_ids, function(x){
...
PHYLOPLOT <- function(input, output, session, data, type = "lineage"){
  ...
  observe({
    if(input$button_apply_phylotree_map >0 ){
      ...
      tree_df$pathString = apply(tree_df[1:length(tree_df) -1], 1, function(x) paste(x, collapse = "/"))
  ...
  observe({
    if(input$button_plot_sunburst >0 ){
      ...
      isolate({
        ...
        pathString = apply(tree_df[1:(ncol(tree_df)-1)], 1, function(x) paste(x, collapse = "-"))
  ...
  fluidRow(
    tabBox(
      ...
      tabPanel("Statistics",
               ...
               sliderInput(ns("p_cut_off"), "P-value cut-off", 0.001, 1, 0.05, step = 0.001),
               sliderInput(ns("fc_cut_off"), "Fold-change cut-off", 1, 5, 1.5, step = 0.1),
               ...
               conditionalPanel(
                 ...
                 sliderInput(ns("curvature"), "Curvature", 0.01, 1, 0.2, step = 0.01)
               ),
    ...
    tabBox(
      ...
      tabPanel("Result table download",
               tableDownloadUI(ns("download_result_table"), # make sure to use ns, because this is moduel in module,
...
volcano_plot_server <- function(input, output, session, data, meta){
  ...
  stats_t <- reactive({
                stats_t <- lapply(1:nrow(data_combined()), function(x) t.test(data_combined()[x, 1:ncol(data1())], data_combined()[x, (ncol(data1()+1)):ncol(data_combined())]))
                names(stats_t) <- row.names(data_combined())
                return(stats_t)
             })

  stats_wc <- reactive({
                stats_wc <- suppressWarnings(lapply(1:nrow(data_combined()), function(x) wilcox.test(data_combined()[x, 1:ncol(data1())], data_combined()[x, (ncol(data1())+1):ncol(data_combined())])))
  ...
  log_p_as_y_axis <- reactive({
    if(input$Hypothesis == "Parametric"){
      if(input$p_adjust == "none"){
        p.value.t <- lapply(1:nrow(data_combined()), function(x) stats_t()[[x]]$p.value)
      ...
      if(input$p_adjust == "none"){
        p.value.wc <- lapply(1:nrow(data_combined()), function(x) stats_wc()[[x]]$p.value)
  ...
  log2FC <- reactive({
                      log2FC <- lapply(1:nrow(data_combined()), function(x) log2(mean(data_combined()[x, 1:ncol(data1())])/mean(data_combined()[x, (ncol(data1())+1):ncol(data_combined())])))
  ...
  intensity <- reactive({
    if(input$show_intensity == "no"){
      ...
      intensity <- lapply(1:nrow(data_combined()), function(x) mean(data_combined()[x,]))
      names(intensity) <- row.names(data_combined())
      intensity <- as.matrix(t(as.data.frame(intensity)))
      return(intensity)
    }else if(input$show_intensity == "1"){
      intensity <- lapply(1:nrow(data1()), function(x) mean(data1()[x,]))
      names(intensity) <- row.names(data1())
      intensity <- as.matrix(t(as.data.frame(intensity)))
      return(intensity)
    }else{
      intensity <- lapply(1:nrow(data2()), function(x) mean(data2()[x,]))
  ...
  data_output <- reactive({
                 data_output <- as.data.frame(cbind(log2FC(), 10^(-log_p_as_y_axis()), mycolorg()))
                 names(data_output) <- c("Log2 fold-change", "p value", "Change")
                 return(data_output)
  })
  ...
  callModule(tableDownload,"download_result_table", 
             data = data_output(), 
             filename_tag = "result_table")
